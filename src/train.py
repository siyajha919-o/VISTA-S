...
def main():
    try:
        ...
        data_config = os.path.join(os.path.dirname(
            __file__), '../data/raw/yolo_params.yaml')

        if not os.path.exists(data_config):
            raise FileNotFoundError(
                f"Configuration file not found: {data_config}")
        ...
        print(
            f"Error: {e}. Please install required packages with: pip install ultralytics")
        sys.exit(1)
    except Exception as e:
